class FullMeritScholar extends Scholar implements renewBenefit, PrintableNotice {
    private final double GWA_Req = 1.75;
    private final int Min_srvcHour = 20;
    private final int Max_absence = 3;

    public FullMeritScholar(String name, String scholarID, double gwa, double serviceHoursTotal, int attendanceIssues) {
        super(name, scholarID, gwa, serviceHoursTotal, attendanceIssues);
    }

    @Override
    public String getScholarCategoryName() {
        this.scholarType = "Full Merit Scholarship";
    ...
    public void buildDecisionReason() {
        if (gwa > GWA_Req && attendanceIssues > Max_absence && serviceHoursTotal < Min_srvcHour) {
            ...
            adviserNote = "Student lacks the minimum hours of service.";
        } 
        else if (warning_count > 1) {
            currentStatus = "FOR REVIEW";
            renewalResult = "Disciplinary warnings exceeding limit.";
            adviserNote = "Student has a warning count. Clean record must be maintained.";
        }
        else {
